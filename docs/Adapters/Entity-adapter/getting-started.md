...
Para configurar um EntityAdapter no Fusion, abra as configurações do formulário em **Formulários - Configurar - Dinâmicos** selecione o formulário que deseja colocar o adapter de
customização.
...
Nesse **map** contem o formulário utilizado para fazer a customização necessária.
...
Para uma implementação simples, basta apenas saber em qual formulário deseja criar o Adapter, criar uma classe java no diretório desejado e implementar a interface **EntityAdapter
**, dar **@Override** no método **run()** conforme o fonte abaixo.
...
No adapter criado a cima temos um exemplo de uma implementação simples, onde foi pego o EForm do map *((EForm) map.get("eform"))*, o NeoObject do formulario *(
formulario.getObject();)* e criado um Entity Wrapper do objeto *(new EntityWrapper(noFormulario);)*.
Com o *wrapper* foi possivel acessar o campo *(ewFormulario.findField("implementacaoSimples"))* e inserir um valor *(.setValue(true);)*.
...
- Os Adapters são geralmente criados dentro do diretório "com.neomind.fusion.workflow.adapter".
...
- Acessar o Gerenciamento de Modelos de Processos em **Processos - Configurar - Gerenciar Modelos de Processos** e criar um modelo novo ou editar um existente, esse processo será
  um subprocesso em nosso processo principal.
...
- Acessar a central de formulários em **Formulários - Central de Formulários - Novo Formulário**. Criar um novo formulário do nosso novo processo. Lembre-se de criar nesse
  formulário um campo do tipo E-form que referencie o formulário principal do nosso processo principal.
...
- O checkpoint consiste em dois adapters que executam o subprocesso via código, e não por fluxo normal de processo. Para isso ocorrer, é necessaria inserção uma condição
  impossivel no fluxo do adapter para o subprocesso.
...
- *Inicia Subprocesso* para iniciar os sub processos:
    - Para iniciar os subfluxos é necessário passar o nome do subprocesso no modelo (Sub Processo Aprovação), a última task de usuário, o formulario do processo e o codigo do campo
      do formulario que representa a **Lista de Entidades**.
...
- O método 'startSubProcess()' é o gatilho que inicia o subprocesso. Atenção para:
    ...
    - O ultimo parâmetro, que recebe o código do campo do formulário principal e referência o subprocesso, contendo a lista de NeoObjects que irá iniciar os sub processos. Para
      cada registro nessa lista, será criado um subfluxo diferente do subprocesso.
...
public class FinalizaAprovacoes implements AdapterInterface
{
	...
	public void start(Task task, EntityWrapper entityWrapper, Activity activity)
	{
		new CheckPointUtils().checkFinishNew(entityWrapper, activity, "RegistrosAprovacoes", "Sub Processo Aprovação", "Aguarda Até Todos Finalizarem");
	}
...
- Assim que o checkpoint tenha executado um subfluxo para cada registro na **Lista de Entidades**, o *Adapter final* irá finalizar o checkpoint e seguirá o fluxo normal do processo
  principal

- Atenção para o último parâmetro do método 'checkFinishNew()', que recebe o nome da atividade de Script deste adapter